...
Sunroof represents functions with the type 
...
the context of JavaScript, we only permit uncurried functions.
Thus a \Src{Sunroof $\alpha$} constraint would prohibit 
functions to take more than one argument.
We introduced \Src{SunroofArgument} to constrain the 
...
We can create functions with the \Src{function} combinator.
\begin{Code}
...
restricted to the second threading model. Continuations

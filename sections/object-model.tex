...
users can write their one objects. This means our 
system to type JavaScript needs to be extensible.

The approach from \cite{Svenningsson:12:CombiningEmbedding}
...
phantom types to give more specific types (TODO: cite phantom types).
...
\Src{ValueOf} and offers the function \Src{js} to convert to that type.

For most types this conversion can be done without side-effects
in JavaScript, since many of basic types are represented as 
immutable objects. An exceptions to this rule is \Src{JSFunction},
because creating a function captures an observable allocation,
e.g. two syntactically equal function are different according to reference
equality. So we have to ensure that a function is not inlined, but 
bound to a new variable when created so that Haskell semantics are 
not violated (TODO: This sentence is crap).
Since values in Sunroof are supposed to be immutable, as in Haskell, 
we can not create arrays through \Src{SunroofValue} 
(TODO: Better semantics for SunroofValue).

We also want to work with record like data structures in JavaScript.

...
the case for \Src{JS\_Assign\_}. Recall its signature from 
Figure \ref{fig:jsi-definition}:
\begin{Code}
JS_Assign_ :: (Sunroof a) => Id -> a -> JSI t ()
\end{Code}
This instruction assigns the value \Src{a} to the 
variable given by the \Src{Id}, which is just a synonym for \Src{String}.
\begin{Code}
...
statement and the code inside of \Src{rest} refers to them

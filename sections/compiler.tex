...
The \Src{Program} type is abstract. Before we 
can work with it we need to convert it to a \Src{ProgramView}
instance.
The monadic structure captured by \Src{ProgramView} can be taken 
apart to translate the instructions inside.
\begin{Code}
...
compile p = eval (view p)
  where
    ...
    ...
\end{Code}
The compiler options and fresh variables are provided 
through the \Src{CompM} monad.

Lets see how \Src{JS\_Assign\_} is translated. Recall from 
Figure \ref{fig:jsi-definition} that it takes a variable name 
and the value to assign as arguments. The result type is unit.
\begin{Code}
...
Next we compile that happens after the assignment. Since 
...
A more interesting part of the compiler is the translation of branches.
...
\end{figure}
First we generate the statements for the branching condition.
...
are not visible outside of the 
it. That means the variables with the result are not visible 
...
How can we fix this? If we are working within the blocking 
...
%provides the threading model currently used. 
%%% KNOWN FROM PREVIOUS SECTION
...
The string argument provides the name of the variable to 
bind the computation result to.

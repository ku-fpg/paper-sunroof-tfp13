...
These ideas given here are not new, though the use of monadic reification
for JavaScript generation is original. What we demonstrate is how
far the science of DSL capture has come, using a full scale example.
...
|boolean| provides the capability to express control-flow in an embedded DSL, 
as needed by Sunroof.
Further, \Src{JSString} is a \Src{Monoid}, which is only possible because JavaScript
strings are immutable values, providing a neat and standard string concatenate operator for Sunroof.
...
instance Monad (JS t) where
  ...
  (Bind va h) >>= k = Bind va (\ a -> h a >>= k)  -- assoc
\end{Code}  
...
We have written other larger applications in Sunroof,
including a pdf-viewer which support marking up exam scripts in a systematic way,
but the small calculator is a representative example.
...
and client side. The programmer is not restricted by an interface or language 
barrier. If the programmer need the client to do something, you can just 
